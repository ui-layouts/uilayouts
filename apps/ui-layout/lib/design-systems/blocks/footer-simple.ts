...
export const footer_simpleDesign = defineDesignSystem({
  name: 'Footer Simple',
  personality:
  ...
  effects: [
    {
      name: 'Plain text footer',
      recipe:
        'Use white, black and muted zinc link text with one nearly invisible separator; no shadow, gradient, image or motion.',
      usage: 'ending utility-focused pages without stealing attention',
    },
    {
  ...
  extensionRules: [
    'Carry the **Plain text footer** into at least one meaningful focal area; reproduce its layer recipe rather than substituting a generic gradient.',
